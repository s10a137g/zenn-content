...
topics: ["bigquery", "salesforce", "googlecloud", "datatransferservice", "etl"]
published: false
---

BigQuery Data Transfer Service（DTS）のドキュメントを読んでいたら、Salesforce 連携の転送モードに「全量投入」と「差分投入（プレビュー）」の2つがあることに気づきました。
...
| 差分投入 Append | 新規行だけを追加 | 作成時のみ更新されるウォーターマーク列 |
...
まず Salesforce 側に検証用の取引先データを用意しました。
...
差分投入では、変更を検知するためのウォーターマーク列と主キーを指定します。今回はウォーターマーク列に `SystemModstamp`（TIMESTAMP 型）、主キーに `Id` を指定しました。
...
差分投入が効くのはコスト面でも同じです。DTS のサードパーティコネクタは、GA 到達後はコンピュートリソース（スロット時間）に応じた従量課金になります。毎回全件を取り直す全量投入より、変更分だけの差分投入のほうが転送量が少なく、その分有利です。

なお、Salesforce の増分取り込みは現在 Preview で、プレビュー中のコネクタ利用は無料です。個人で試す範囲では、Salesforce（Developer Edition）も BigQuery の無料枠も含めてほぼ0円で一通り体験できます。
